...
- **You choose `nodeId`, and it is required.** It is how the graph wires itself together: a transition points at a node through `toNodeId`, which must equal that node's `nodeId`.
- Node IDs must be **unique** within the flow and are capped at **20 characters**.
...
- If you omit a `nodeId`, the request is rejected. There is no auto-generation for nodes: the graph cannot connect nodes it cannot name.
