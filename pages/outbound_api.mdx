...
<Steps>
  <Step title="Obtain an API Key">
    Secure an API key from NLPearl.AI to authenticate and interact with our outbound call API.
    </Step>
  <Step title="Create Outbound Activities">
    Use the platform to define and create an outbound activity for API use. The process is the same as creating any outbound activity managed by the platform.
  </Step>
  <Step title="Trigger Calls via API">
    Once the outbound activity is created, use the API to trigger calls independently of the platform's schedule. This allows for dynamic call initiation based on real-time application logic.
    </Step>
</Steps>
...
For more information about making API-triggered calls, refer to the [Make Call API Request](/api-reference/v1/outbound/make-call) documentation.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="phone"
    iconType="light"
    href="/api-reference/v1/outbound/make-call"
  >
